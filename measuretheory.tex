  ...
  \begin{lemma}
    ...
    \item Let $(p_n)_{n\in\NN}$ an enumeration of the rational numbers in
      ...
      \note[item]<3->{First the left inclusion: Let $x \in [0,1]$ and $\bar x$
        ...
        and $y-x = p_j \in \QQ$. Done. The right hand inclusion is trivial.}
    \end{enumerate}
  \end{lemma}
\end{frame}
...
\begin{frame}{$\sigma$-algebras}
  ...
  \note[item]<2->{The Borel algebra is maybe the most important example, as it
    ...
    also play an important role in probability theory on $\RR^d$.}
...
\begin{frame}{Measures}
  \begin{definition}[measure]
    ...
    \end{enumerate}
  \end{definition}
  \note[item]{A measure is the base object for things like volume functions or
    later also probabilities}
  \begin{example}
    \begin{enumerate}
