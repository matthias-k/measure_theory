...
\begin{frame}{The vitali set}
  ...
  \note[item]{First, we need two simple properties of the vitali set}

  \begin{lemma}
    \begin{enumerate}
    \item For $p\neq q$, $p,q\in\QQ$ we have $p+V \cap q+V = \emptyset$.
      \note[item]{The first is, that arbitrary shifts of the vitali set by an
    ...
    \item Let $(p_n)_{n\in\NN}$ an enumeration of the rational numbers in
      ...
      \note[item]{This one says that the union of all these shifts contain the
        unit interval and are contained in the interval $[-1,2]$.}
      \note[item]{First the left inclusion: Let $x \in [0,1]$ and $\bar x$ its
        class in $\RR/\QQ$. Then there is an $y \in \bar x$ with $y \in V$ and
        $y-x = p_j \in \QQ$. Done. The right hand inclusion is trivial.}
    \end{enumerate}
  \end{lemma}
\end{frame}
...
\begin{frame}{The Vitali set}
  ...
  \note[item]{Now we can prove with the vitali set what we cannot have a
  ...
  \note[item]<2->{As a volume has to be monotone (as I told you before), we get by
    $\sigma$-additivity that sum of the volumes of all the $V_n$ has to be
    between $1$ and $3$.}
  ...
  \note[item]<3->{But as the $V_n$ are just shifts of the vitali set, they have
    to have all the same volume, thus we get that the countable sum over
